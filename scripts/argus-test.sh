...
- The browser console is only for confirming a UI bug you already observed. Do not spend turns analysing console logs, network requests or backend endpoints on their own. (Exception: an HTTP 5xx surfaced automatically as new_server_errors in an action result IS a bug to record — that signal comes to you, it is not spelunking.)
