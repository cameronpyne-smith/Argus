...
- Write /opt/data/run/summary.md with write_file: what you did this run, and for EACH checklist item (1-5) state whether you attempted it, skipped it as not-applicable, or hit a blocker, and what happened.
- Print a short list of the bugs you recorded as your final message.

Do NOT write summary.md while an applicable item is still both unattempted and unblocked — but once every applicable item is either attempted or blocked, finish promptly. summary.md means the deep checklist is DONE; do not keep re-testing a blocked feature to burn turns." \
  $PROVIDER_FLAGS || echo "⚠ agent session exited abnormally — continuing with post-run"
fi
